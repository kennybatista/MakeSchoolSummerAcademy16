 ...
 */
//So, if there is no default value for the property, we must initialize it, using init()
//An initializer allows you to pass in some parameters to set up the initial state of the class as it's being constructed.
...
class Car: Vehicle {
    ...
    init(bodyStyle: String) {
        // 3
        //we use self, so the compiler doesn't get confused about
...
//Structs are for us to group related data
//Structs unlike Classes, do not have inheritance
//If we don't expect to use subclasses, then use structs
...
class Students {
    var name = ""
    var classYear = ClassYear.Freshmen
}
