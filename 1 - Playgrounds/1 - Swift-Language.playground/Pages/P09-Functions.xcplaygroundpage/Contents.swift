 ...
 */
func doNothing(/* no paramaters in these parentheses*/) {
...
// the function's body only sees the internalName
// the caller only sees the externalName
...
areaOfSquare(10, width: 10)


func perimeterOfSquare(side1: Int, side2: Int, side3: Int, side4: Int) -> Int{
    let perimeter = side1 + side2 + side3 + side4
    return perimeter
}

perimeterOfSquare(10, side2: 10, side3: 10, side4: 10)

func irregularPerimeter(sides: [Int]) -> Int {
    var totalLengths : Int = 0
    for sidess in sides {
        totalLengths += sidess
    }
...
//function, that will take in a prefix, then if it finds those names with those prefixes output them
func findInstructorsBy(prefix: String) -> [String]? {
    //emtpy array that will hold the array of found names
    var foundInstructors = [String]()
    //loops through the names to see if it finds a name with the prefix
    for instructor in instructors {
        //if it finds a name with a prefix, do this:
        if instructor.hasPrefix(prefix) {
            //add the found prefixes to the array
